...
const UpdateProduct = () => {
  ...
  const { updateExistingProduct } = useContext(ProductsContext);

  const productToEdit = location.state?.product || null;

  const [productName, setProductName] = useState(productToEdit?.name || "");
  const [price, setPrice] = useState(productToEdit?.price || "");
  const [stock, setStock] = useState(productToEdit?.quantity || "");
  const [category, setCategory] = useState(productToEdit?.category || ""); // new state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateExistingProduct(productToEdit.id, productData);
      alert("Product updated successfully!");
      navigate("/products");
    } catch (err) {
      console.error(err);
      setError(
        err.response?.data?.message || "Failed to update product! Check console for details."
      );
    }
  };
